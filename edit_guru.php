...
$query = mysqli_query($db, "SELECT guru.*, user.username FROM guru LEFT JOIN user ON guru.id_user = user.id WHERE guru.id = '$id'");
...
if (isset($_POST['ajax'])) {
    ...
    if ($update && !empty($username)) {
        if ($row['id_user']) {
            ...
            mysqli_query($db, "INSERT INTO user (username, password, role) VALUES ('$username', '$pass_md5', 'guru')");
            $new_id_user = mysqli_insert_id($db);
            mysqli_query($db, "UPDATE guru SET id_user='$new_id_user' WHERE id='$id'");
        }
    }
    if ($update) {
                ...
                <div class="bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-sm">
                    <form action="" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_guru" value="<?= $row['nama_guru']; ?>" pattern="[a-zA-Z\s\.,\-']+" oninput="this.value = this.value.replace(/[^a-zA-Z\s\.,\-']/g, '')" title="Nama hanya boleh berisi huruf, spasi, titik, koma, tanda hubung, dan petik" required class="block w-full text-sm rounded-xl border border-slate-200 p-3 bg-slate-50 outline-none focus:bg-white focus:border-slate-900 transition-all font-medium">
                        </div>
                        <div>
