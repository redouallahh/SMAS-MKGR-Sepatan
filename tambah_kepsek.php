...
if (isset($_POST['ajax'])) {
    ...
    $insert_user = mysqli_query($db, "INSERT INTO user (username, password, role) VALUES ('$username', '$pass_md5', 'kepala_sekolah')");
...
<head>
    ...
    <title>Tambah Kepala Sekolah - SMAS MKGR Sepatan</title>
        ...
        <div class="flex-1 min-w-0 h-screen overflow-y-auto bg-slate-50/30 relative">
            ...
            <div class="p-6 md:p-10 max-w-[800px] mx-auto space-y-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-xl font-extrabold text-slate-900">Registrasi Kepala Sekolah</h2>
                        <p class="text-xs text-slate-400 mt-0.5">Tambahkan biodata kepala sekolah ke dalam database master.</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-sm">
                    <form action="" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_kepsek" pattern="[a-zA-Z\s\.,\-']+" oninput="this.value = this.value.replace(/[^a-zA-Z\s\.,\-']/g, '')" title="Nama hanya boleh berisi huruf, spasi, titik, koma, tanda hubung, dan petik" placeholder="Contoh: Drs. Ahmad Subarjo, M.Pd." required class="block w-full text-sm rounded-xl border border-slate-200 p-3 bg-slate-50 outline-none focus:bg-white focus:border-slate-900 transition-all font-medium">
                        </div>
                        <div>
                        ...
                        <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3 mt-4">
                            ...
                            <button type="submit" name="submit" class="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-sm transition-all">Simpan Record Kepala Sekolah</button>
                        </div>
                    </form>
                </div>
