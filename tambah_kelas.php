...
if (isset($_POST['ajax'])) {
    ...
    if (mysqli_num_rows($cek_database) > 0) {
        // Jika nama kelas sudah ada
        echo json_encode(['status' => 'error', 'message' => "Gagal! Nama kelas '$nama_kelas' sudah terdaftar di sistem."]);
    } else {
        // 2. Jika aman dan belum ada, eksekusi INSERT
        ...
        if ($insert) {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Terjadi kesalahan sistem: ' . mysqli_error($db)]);
        }
    }
    exit;
}
            ...
            <div class="p-6 md:p-10 max-w-[600px] mx-auto space-y-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Definisikan ruang dan pengarsipan identitas kelas .</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-sm">
                    <form action="" method="POST" class="space-y-5">
                        <div>
                            ...
                            <input type="text" name="nama_kelas" pattern="[a-zA-Z0-9\s\-]+" oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '')" title="Nama kelas hanya boleh berisi huruf, angka, spasi, dan tanda hubung" placeholder="Contoh: X IPA 1 / XI IPS 2" required class="block w-full text-sm rounded-xl border border-slate-200 p-3 bg-slate-50 outline-none focus:bg-white focus:border-slate-900 transition-all font-medium">
                        </div>
